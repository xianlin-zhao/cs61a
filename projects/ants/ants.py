...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            now = place.ant
            if now.is_container() and now.can_contain(self):
                now.contain_ant(self)
            elif self.is_container() and self.can_contain(now):
                self.contain_ant(now)
                place.ant = self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        place = self.place
        dist = 0
        while (not place.is_hive()) and (dist <= self.max_range):
            if dist >= self.min_range:
                ans = bee_selector(place.bees)
                if ans is not None:
                    return ans
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount):
        ...
        bees = place.bees[:]
        for bee in bees:
            id = place.bees.index(bee)
            place.bees[id].reduce_health(amount)
        super().reduce_health(amount)
        if self.health <= 0:
            bees = place.bees[:]
            for bee in bees:
                id = place.bees.index(bee)
                place.bees[id].reduce_health(self.damage)
        # END Problem 5
...
class TankAnt(ContainerAnt):
    ...
    def action(self, gamestate):
        ...
        for bee in bees:
            id = self.place.bees.index(bee)
            self.place.bees[id].reduce_health(self.damage)
# END Problem 9
...
# END Problem EC
    ...
    def reduce_health(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        super().reduce_health(amount)
        if self.health <= 0 and self == QueenAnt.true_queen:
            bees_win()
        # END Problem EC

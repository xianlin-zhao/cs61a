...
def accuracy(typed, reference):
    ...
    if len1 == 0 and len2 == 0:
        ...
        return 0.0
    length, acc = min(len1, len2), 0
    for i in range(length):
        if typed_words[i] == reference_words[i]:
            acc += 1
    return acc * 100.0 / len1
    # END PROBLEM 3
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        return typed_word
    dist = [diff_function(typed_word, word, limit) for word in valid_words]
    nowmin = min(dist)
    if nowmin > limit:
        return typed_word
...
def sphinx_switches(start, goal, limit):
    ...
    def func(s, g, now):
        ...
        if len(s) == 0 or len(g) == 0:
            return now + abs(len(s) - len(g))
        if s[0] == g[0]:
            return func(s[1:], g[1:], now)
        return func(s[1:], g[1:], now + 1)
...
def pawssible_patches(start, goal, limit):
    ...
    def func(s, g, now):
        ...
        if s[0] == g[0]:
            return func(s[1:], g[1:], now)
        return min(func(s[1:], g[1:], now + 1), func(s, g[1:], now + 1), func(s[1:], g, now + 1))
    return func(start, goal, 0)
...
def fastest_words(game):
    ...
    for i in word_indices:
        tmp = [time(game, j, i) for j in player_indices]
        tmp_min = min(tmp)
        winner_per_word.append(tmp.index(tmp_min))
    ans = [[] for _ in player_indices]
    for i, winner in zip(word_indices, winner_per_word):
